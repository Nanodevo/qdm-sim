...
for x, lab in ((material.C13_NATURAL, "natural ¹³C"), (1e-4, "¹²C-enriched")):
    ...
    i = int(np.argmin(eta)); a.plot(nppm[i], eta[i], "ko", ms=4); a.text(nppm[i] * 0.8, eta[i] * 0.78, f"optimum ≈ {nppm[i]:.1f} ppm", fontsize=8, ha="right", va="top")
a.set(title="(c) CW sensitivity per 0.5 µm pixel vs [N]: more NVs against shorter T₂*", xlabel="[N] (ppm)", ylabel="sensitivity (nT/√Hz)")
a.set_ylim(1.0e2, 2.6e3); a.legend(loc="upper right")
...
fig.tight_layout(); fig.savefig(OUT / "fig4_material.png"); print("fig4")
...
a.axvline(2, color="k", ls=":", lw=0.8); a.text(28, 0.12, "dotted line: optimum s = 2,\nwidth ×√3, contrast ⅔", fontsize=8, ha="right", va="bottom")
...
fig.tight_layout(); fig.savefig(OUT / "fig5_protocols.png"); print("fig5")
